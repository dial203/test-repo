...
/// SHA-256, FIPS 180-4.
///
/// Hand-rolled rather than CryptoKit so the analysis core stays free of Apple-only
/// frameworks and the configuration fingerprint below can be computed — and tested —
/// identically on device and in Linux CI. This is used for content addressing, never for
...
public struct SHA256 {
    ...
    public static func digest(_ message: [UInt8]) -> [UInt8] {
        ...
        // Pad: 0x80, zeros, then the length in bits as a big-endian UInt64.
        var padded = message
    ...
    public static func hex(_ data: Data) -> String {
        digest([UInt8](data)).map { String(format: "%02x", $0) }.joined()
    }
